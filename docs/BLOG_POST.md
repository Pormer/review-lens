...
![리뷰렌즈 상품 분석 화면](images/home-desktop.png)
...
![리뷰렌즈 예제 리포트](images/report-mobile.png)
...
실제 상품 분석은 Ollama와 `qwen2.5:3b` 모델을 설치한 뒤 사용할 수 있으며 유료 API 키가 필요 없다. 최초 모델 다운로드에 수 GB의 저장 공간이 필요하고 처리 속도는 PC 사양에 따라 달라진다. Docker·Compose와 예제 평가용 Render 설정도 포함했다. 작은 무료 호스팅에서 로컬 모델까지 실행된다고 주장하지 않는다. 모델 실동작과 공개 서비스 배포 상태는 저장소의 검증 문서에서 확인할 수 있다.
...
태그 제안: Python, FastAPI, AI에이전트, OpenAI, 사이드프로젝트, 리뷰분석

게시 준비 메모(발행 전 삭제): 티스토리에는 이미지 파일을 직접 첨부해 위 상대 경로를 교체한다. 현재 글은 구현 범위와 외부 검증의 한계를 구분한 초안이며, 배포 URL이 확정되면 본문에 추가한다.
