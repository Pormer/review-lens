...
async def run_local(request, settings, stage):
    if not await local_ready(settings):
        raise ValueError("local_model_unavailable")
    sources, records, limitations = await collect(request, stage)
    evidence, excluded = [], 0
    ...
    for index, record in enumerate(records[:6]):
        await stage(f"로컬 AI · 상품 일치와 근거 추출 {index + 1}/{min(len(records), 6)}")
        ...
        try:
            ...
            limitations.append(f"{source.id}: 로컬 모델의 추출 응답을 검증하지 못해 해당 자료를 제외했습니다.")
    if evidence:
        ...
        excluded += dropped
        await stage("로컬 AI · 장단점과 사용 경험 정리")
    ...
    limitations += [
        "무료 웹 검색과 공개 페이지의 부분 자료를 로컬 AI로 분석했습니다. 전체 쇼핑몰 리뷰를 수집한 결과가 아닙니다.",
        "상품 일치·후기 유형·감성은 로컬 AI의 추정입니다. 검색 발췌는 원문 맥락이 부족할 수 있으므로 출처를 확인해 주세요.",
