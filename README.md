...
**현재 제공 범위: 로컬 실행 버전.** 공개 호스팅과 쇼핑몰별 수집 확장은 보류했습니다. AI는 Ollama를 기본으로 사용하며, OpenAI는 직접 설정할 때만 사용합니다. [로컬 사용·재실행 안내](docs/LOCAL_USE.md). 상품 분류·요약 정확도와 원문 수집에는 알려진 한계가 있습니다.
...
![리뷰렌즈 화면](docs/images/home-1920x1080.png)

## 현재 제공하는 기능

- **NAVER API HUB 공식 검색**: 키 설정 시 블로그·웹문서 탐색으로 대체하며 OpenAI 내장 웹 검색을 생략합니다. 종료된 쇼핑 검색 API는 사용하지 않습니다. [발급·비용과 기능별 대체 범위](docs/OFFICIAL_APIS.md)
...
기본 로컬 분석은 무료 검색 최대 3회, 후보 출처와 원문 수집 시도 최대 10개, 모델 추출 최대 6개 출처를 대상으로 합니다. 본문을 확보한 페이지를 우선 분석합니다. 기본 전체 제한은 작업당 600초, 실제 분석 시간당 10건입니다. 검색 결과·접근 정책·모델 출력 품질에 따라 근거를 확보하지 못할 수 있습니다. 리포트에는 발견한 출처·본문 확보·AI 검토·검증 통과 근거 수가 표시됩니다.
